...
func TestClaudeVertexChat(t *testing.T) {
	project := getEnv(t, "GCP_PROJECT")
	provider, err := llm.NewClaudeVertex(project, getEnv(t, "GCP_LOCATION", "us-east5"), 3)
	if err != nil {
...
func TestClaudeVertexChatStream(t *testing.T) {
	project := getEnv(t, "GCP_PROJECT")
	provider, err := llm.NewClaudeVertex(project, getEnv(t, "GCP_LOCATION", "us-east5"), 3)
	if err != nil {
		t.Fatalf("create provider: %v", err)
...
func TestClaudeVertexChatCache(t *testing.T) {
	project := getEnv(t, "GCP_PROJECT")
	provider, err := llm.NewClaudeVertex(project, getEnv(t, "GCP_LOCATION", "us-east5"), 3)
	if err != nil {
		t.Fatalf("create provider: %v", err)
	...
	req := llm.ConversationRequest{
		...
		Params:       map[string]any{"max_tokens": 256, "temperature": 0.0},
	}
